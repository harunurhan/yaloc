...
describe('LoadingCache', () => {
  it('loads the value for a key if it does not exist', async () => {
    const cache = new LoadingCache<string, string>((key) => Promise.resolve(`value-of-${key}`));
  ...
  it('propagates the error that is thrown during loading', async () => {
    const cache = new LoadingCache<string, string>((key) => { throw new Error(`can not load "${key}"`) });
    try {
      await cache.get('foo');
    } catch (error) {
  ...
  it('propagates the error if load function rejects with an error', async () => {
    const cache = new LoadingCache<string, string>((key) => Promise.reject(new Error(`can not load "${key}"`)));
    try {
      await cache.get('foo');
    } catch (error) {
      expect(error.message).toEqual('can not load "foo"');
